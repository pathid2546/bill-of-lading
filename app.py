...
with tab_upload:
    file = st.file_uploader("ส่งไฟล์ Excel มาเลยค่ะคุณเดียร์ เดี๋ยวจัดการให้กริบ!", type=["xlsx"])
...
if file:
    try:
        ...
        if header_idx is not None:
            ...
            with tab_process:
                if st.button("🚀 ประมวลผลไฟล์ตัวจบ (แถบสีเทา TOTAL) !"):
                    ...
                    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
                        ...
                        # [ส่วนหน้าอื่นๆ: ป้ายน้ำหนัก, ป้ายกล่อง คงเดิม]
                        # 1. ป้ายน้ำหนัก
                        ...
                        # --- 3. หน้าน้ำหนัก (Highlight Total สีเทา) ---
                        ...
                        # --- บรรทัด TOTAL (ปรับเป็นสีเทาตามสั่ง) ---
                        t_row = len(m_weight) + 2; ws3.set_row(t_row, 35)
                        ws3.write(t_row, 0, "", s_f_21); ws3.write(t_row, 1, "", s_f_21) # ช่อง No, Trip เป็นสีเทา
                        ws3.write(t_row, 2, "TOTAL", s_f_21_left)
                        d_idx = 3
                        for p in meat_items:
                        ...
                        ws3.write(t_row, d_idx, "", s_f_21); ws3.write(t_row, d_idx+1, "", s_f_21) # ตะกร้า, กล่อง เป็นสีเทา
                        ...
                        # TOTAL หน้าจัดกล่อง
                        l_row = len(m_box) + 1; ws4.set_row(l_row, 35); ws4.write(l_row, 0, "", s_f_21); ws4.write(l_row, 1, "", s_f_21); ws4.write(l_row, 2, "TOTAL", s_f_21_left)
                        for idx, col in enumerate(cols_b):
                    ...
                    st.balloons()
                    st.download_button(label="💖 ดาวน์โหลดไฟล์ที่ปรับสีเทา TOTAL แล้วค่ะเดียร์! 💖", 
                                     data=output.getvalue(), 
                                     file_name=f"Queen_Logistics_Grey_Total_{datetime.now().strftime('%H%M')}.xlsx")
