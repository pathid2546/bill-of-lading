...
with tab_upload:
    file = st.file_uploader("ส่งไฟล์ Excel มาเลยค่ะคุณเดียร์ แก้ไขชื่อซ้ำในวงเล็บให้เรียบร้อย!", type=["xlsx"])
...
if file:
    try:
        ...
        for _, r in route_df.iterrows():
            ...
            combined = code
            # 🔥 แก้ไขตรงนี้: เช็คว่าถ้า order_no มันซ้ำกับ code ก็ไม่ต้องเอามาต่อท้ายให้เบิ้ล
            if order_no and order_no != code:
                combined += f" {order_no}"
        ...
        if header_idx is not None:
            ...
            with tab_process:
                if st.button("🚀 ประมวลผลระบบครบจบ (แก้ชื่อสาขาเบิ้ลเรียบร้อย)"):
                    ...
                    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
                        ...
                        for _, row_s in m_weight.iterrows():
                            ...
                            for i, item in enumerate(meat_items):
                                ...
                                ws1.write(r, 4, "กล่อง" if "ออส" in item or "คูโร" in item or "เนื้อวัว" in item else "ตะกร้า", f_unit_v)
                                ws1.set_row(r, 62)
                            row_idx += len(meat_items) + 2; breaks_w.append(row_idx)
